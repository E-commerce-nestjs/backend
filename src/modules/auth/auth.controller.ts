...
export class AuthController {
  ...
  @ApiResponse({status:HttpStatus.CREATED,description:"Register successfully",type: createAppResponseDto(RegisterResponseDto,{code:HttpStatus.CREATED,message:"Register successfully"})})
  @ApiResponse({status:HttpStatus.BAD_REQUEST,description:"Validation failed or user already exists",type: ValidationErrorResponseDto})
  async register(@Body() registerDto : RegisterDto):Promise<AppResponseData<Omit<User,'password'>>>{    
    const user = await this.authService.register(registerDto)
    return AppResponse.ok<Omit<User,'password'>>(user,"Register successfully",HttpStatus.CREATED)
  }
  ...
  @ApiResponse({status:HttpStatus.UNAUTHORIZED,description:"Invalid credentials",type: UnauthorizedErrorResponseDto})
  async login(@Body() loginDto:LoginDto, @Res({ passthrough: true }) res:Response):Promise<AppResponseData<LoginResponseDto>>{
    ...
    @ApiResponse({status:HttpStatus.UNAUTHORIZED,description:"Invalid or expired refresh token",type: UnauthorizedErrorResponseDto})
    ...
    @ApiResponse({status:HttpStatus.OK,description:"User retrieved successfully",type: createAppResponseDto(UserResponseDto,{code:HttpStatus.OK,message:"User retrieved successfully"})})
